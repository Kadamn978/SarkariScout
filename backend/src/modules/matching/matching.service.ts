...
export class MatchingService {
  ...
  async scoreJobForUser(userId: string, jobId: string): Promise<MatchResult> {
    const [profile, job] = await Promise.all([
      this.prisma.profile.findUnique({ where: { userId } }),
      this.prisma.job.findUnique({ where: { id: jobId } }),
    ])
    if (!profile || !job) throw new NotFoundException('Profile or job not found')
    return this.matchJob(profile, job)
  }
  ...
  async findMatchingJobs(userId: string): Promise<MatchResult[]> {
    ...
    return results.filter((r) => r.eligible).sort((a, b) => b.matchScore - a.matchScore)
  }
  ...
  private checkEducation(
    ...
    const lowestRequired = Math.min(
      ...jobLevels.map((jl: string) => AGE_LEVELS.indexOf(jl)).filter((i) => i !== -1),
    )
    return {
      score: 0,
  ...
  private checkAge(
    ...
    if (age >= min && age <= max + ageRelief) {
      return {
        score: 10,
        reason: `Age ${age}, max ${max} (+${ageRelief} relaxation for ${profile.category})`,
      }
    }
  ...
  async getJobMatchStats() {
    ...
    const openJobs = await this.prisma.job.count({
      where: {
        status: 'OPEN',
        OR: [{ applyEnd: null }, { applyEnd: { gte: now } }],
      },
    })
