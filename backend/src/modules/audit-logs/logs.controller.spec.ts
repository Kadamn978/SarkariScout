...
describe('LogsController', () => {
  ...
  describe('getLogFile', () => {
    it('should return log file content when file exists', async () => {
      ;(fsPromises.readFile as jest.Mock).mockResolvedValue('line1\nline2\nline3')

      const result = await controller.getLogFile('2026-01-15')
      expect(result).toEqual({ date: '2026-01-15', lines: ['line1', 'line2', 'line3'] })
    })

    it('should throw NotFoundException when log file not found', async () => {
      ;(fsPromises.readFile as jest.Mock).mockRejectedValue(new Error('ENOENT'))

      await expect(controller.getLogFile('2026-01-15')).rejects.toThrow(NotFoundException)
    })
  })
})
