...
export class CompetitorMonitorService {
  ...
  async discoverOfficialSources(): Promise<{
    ...
    this.logger.log(
      `Discovered ${discovered.length} official links, ${newDomains.length} new domains, ${alreadyTracked} already tracked`,
    )

    return { discovered, alreadyTracked, newDomains }
  }

  private async extractOfficialLinks(
    url: string,
    siteName: string,
  ): Promise<{ url: string; name: string; source: string }[]> {
    ...
    while ((match = linkPattern.exec(html)) !== null) {
      ...
      const isOfficial = this.OFFICIAL_DOMAINS.some(
        (od) => domain.endsWith(od) || domain.includes(od),
      )
