...
describe('CrawlerService — Full Run Scenarios', () => {
  ...
  beforeEach(async () => {
    prisma = {
      source: { findMany: jest.fn().mockResolvedValue([]), findUnique: jest.fn(), count: jest.fn().mockResolvedValue(0) },
      job: { findMany: jest.fn().mockResolvedValue([]), upsert: jest.fn(), count: jest.fn().mockResolvedValue(0) },
  ...
  describe('getSourceStats — Positive', () => {
    it('should return source stats array', async () => {
      prisma.source.findMany.mockResolvedValue([{
        id: 's1', name: 'SSC', type: 'RSS', enabled: true,
        lastRunAt: new Date(), lastRunStatus: 'success', itemsPerRun: 10, totalItems: 100,
        crawlLogs: [],
      }]);
