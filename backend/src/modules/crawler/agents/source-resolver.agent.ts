...
export class SourceResolverAgent {
  ...
  private readonly OFFICIAL_SOURCE_MAP: Record<string, { domain: string; path: string }> = {
    ...
    'SBI': { domain: 'sbi.co.in', path: '/web/interest-rates/current-interest-rates/sbv-/sbi-careers' },
  ...
  async resolveAll(jobs: CompetitorJob[], trackedDomains: string[]): Promise<ResolvedJob[]> {
    ...
    this.logger.log(`Resolved ${resolved.length} jobs, ${resolved.filter(r => !r.isAlreadyTracked).length} need new sources`);
    return resolved;
  }
  ...
  private resolveJob(job: CompetitorJob, trackedDomains: string[]): ResolvedJob {
    ...
    if (linkDomain) {
      const isTracked = trackedDomains.some(td => td.includes(linkDomain) || linkDomain.includes(td));
      return {
        ...job,
