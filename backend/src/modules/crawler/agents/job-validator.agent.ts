...
export class JobValidatorAgent {
  ...
  async validateAndInsert(jobs: ScrapedOfficialJob[]): Promise<{
    ...
    this.logger.log(
      `Validation complete: ${inserted} inserted, ${skipped} skipped, ${duplicates} duplicates`,
    )
  ...
  private async insertJob(job: ScrapedOfficialJob): Promise<void> {
    ...
    await this.prisma.job.create({
      data: {
        ...
        eligibilityCriteria: job.description,
        totalVacancies: job.vacancies || null,
        state: job.location || 'ALL_IN',
        category: 'GOVERNMENT',
        status: 'OPEN',
