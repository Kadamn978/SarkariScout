...
export class Logger {
  ...
  log(message: string, meta?: Record<string, any>) {
    const entry: LogEntry = { timestamp: new Date().toISOString(), level: 'info', context: this.context, message, meta };
    this.write(this.logFilePath, entry);
    console.log(`\x1b[32m[${this.context}]\x1b[0m ${message}`);
  }

  warn(message: string, meta?: Record<string, any>) {
    const entry: LogEntry = { timestamp: new Date().toISOString(), level: 'warn', context: this.context, message, meta };
    this.write(this.logFilePath, entry);
  ...
  error(message: string, meta?: Record<string, any>) {
    const entry: LogEntry = { timestamp: new Date().toISOString(), level: 'error', context: this.context, message, meta };
    this.write(this.logFilePath, entry);
    this.write(this.errorFilePath, entry);
  ...
  debug(message: string, meta?: Record<string, any>) {
    ...
    const entry: LogEntry = { timestamp: new Date().toISOString(), level: 'debug', context: this.context, message, meta };
    this.write(this.logFilePath, entry);
  }
