...
async function main() {
  ...
  // Government sources — 34 sources with social media channels
  const sources = [
    // ── Central Government — Major Recruitment Bodies ──
    {
      id: 'ssc',
    ...
    // ── Competitor Monitoring (Not for job scraping, for feature tracking) ──
    {
      id: 'competitor-sarkariresult',
      name: 'SarkariResult.com (Competitor)',
      type: 'HTML' as const,
      baseUrl: 'https://www.sarkariresult.com',
      schedule: '0 0 * * *', // Daily — for monitoring only
      configJson: JSON.stringify({
        isCompetitor: true,
        monthlyVisitors: '50M+',
        founded: 2012,
