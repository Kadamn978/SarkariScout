    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-2">Download previous year question papers for practice.</p>
        </div>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          ...
          {families.map((f) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-medium transition ${filter === f.examFamily ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 border hover:bg-gray-50'}`}
            >
        ...
        <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-sm font-medium whitespace-nowrap transition ${!yearFilter ? 'bg-gray-900 text-white' : 'bg-white text-gray-600 border hover:bg-gray-50'}`}
          >
            All Years
          </button>
          {years.map((y) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-sm font-medium whitespace-nowrap transition ${yearFilter === String(y) ? 'bg-gray-900 text-white' : 'bg-white text-gray-600 border hover:bg-gray-50'}`}
            >
              {y}
            </button>
          ))}
          ...
          <div className="space-y-3">
            {[...Array(8)].map((_, i) => (
              <div key={i} className="bg-white rounded-xl p-5 animate-pulse flex gap-4">
                ...
                <div className="flex-1"><div className="h-4 bg-gray-200 rounded w-3/4 mb-2" /><div className="h-3 bg-gray-200 rounded w-1/4" /></div>
              </div>
            ))}
          </div>
        ) : papers.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-xl">
            <p className="text-gray-500 text-lg">No papers found for this filter.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {papers.map((paper) => (
              <div key={paper.id} className="bg-white rounded-xl p-5 hover:shadow-md transition border border-gray-100 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {paper.description && <p className="text-sm text-gray-400 mt-1 truncate">{paper.description}</p>}
                </div>
                <button
